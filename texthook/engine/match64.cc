...
void MonoCallBack(uintptr_t assembly, void *userData) {
	...
	auto mono_utage_class = mono_class_from_name(image, "Utage", "TextData");
    ...
    if (!mono_tmp_class && !mono_ugui_class && !mono_ngui_class && !mono_rpg_unite_class && !mono_naninovel_class && !mono_utage_class && !mono_schooldays_textwork_class && !mono_advsys_class)
        return;
	...
	if (mono_utage_class) {
		auto mono_method = mono_class_get_method_from_name(mono_utage_class,".ctor",-1);
		if (mono_method) {
			uint64_t *method_pointer = mono_compile_method(mono_method);
			if (method_pointer) {
				HookParam hp = {};
				...
				hp.filter_fun = [](LPVOID data, DWORD* size, HookParam*, BYTE)
				{
					...
					static std::wregex pattern(LR"(<[^>]+>)");
					RegexReplacerW(text, &len, pattern, L"");
					*size = static_cast<DWORD>(len);
					return true;
				};
				hp.length_fun = getV8StringLength;
				ConsoleOutput("Mono_X64,Insert: Utage.TextData.ctor Hook");
				NewHook(hp, "Utage_TextData_ctor");
			}
		}
	}
	...
	*/
	...
	bool InsertWillPlus64Hook() {
		...
		// Tested:
		// OfficeLove -クールな先輩彼女と秘密のイキ抜き- TRIAL EDITION (2.1.0.0)
		// 虜ノ桜 ～美しく咲き、淫らに散らされる処女たち～ 体験版 (2.1.2.0)
		// 天獄のヴィネア Trial Edition (2.2.1.0)
		// 48 8B C4 55 53 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 ?? 48 81 EC ?? ?? ?? ?? 0F 29 70 ?? 0F 29 78 ?? 44 0F 29 40 ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 ?? 4D 8B
		const BYTE bytes1_1[] = {
			0x48, 0x8B, 0xC4, 0x55, 0x53, 0x56, 0x57, 0x41, 0x54, 0x41, 0x55, 0x41, 0x56, 0x41, 0x57, 0x48, 0x8D, 0x6C, 0x24, XX, 0x48, 0x81, 0xEC, XX4, 0x0F, 0x29, 0x70, XX, 0x0F, 0x29, 0x78, XX, 0x44, 0x0F, 0x29, 0x40, XX, 0x48, 0x8B, 0x05, XX4, 0x48, 0x33, 0xC4, 0x48, 0x89, 0x45, XX, 0x4D, 0x8B
		};

		// 有閑夫人倶楽部 -体験版- (2.1.1.0)
		// 48 8B C4 55 53 56 57 41 54 41 55 41 56 41 57 48 8D A8 ?? ?? ?? ?? 48 81 EC ?? ?? ?? ?? 0F 29 70 ?? 0F 29 78 ?? 44 0F 29 40 ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 ?? 4D 8B
		const BYTE bytes1_2[] = {
			0x48, 0x8B, 0xC4, 0x55, 0x53, 0x56, 0x57, 0x41, 0x54, 0x41, 0x55, 0x41, 0x56, 0x41, 0x57, 0x48, 0x8D, 0xA8, XX4, 0x48, 0x81, 0xEC, XX4, 0x0F, 0x29, 0x70, XX, 0x0F, 0x29, 0x78, XX, 0x44, 0x0F, 0x29, 0x40, XX, 0x48, 0x8B, 0x05, XX4, 0x48, 0x33, 0xC4, 0x48, 0x89, 0x45, XX, 0x4D, 0x8B
		};
